...
var getAgentxPaths = utils.getAgentxPaths;
...
var enableAutoStartup = function () {
  var paths = getAgentxPaths();
  debug('paths to create starup script:\n', paths);

  fs.readFile('/usr/local/agentx/startagentx', function (err, stdout, stderr) {
    if (err) {
      throw err;
    }
    var lines = stdout.toString().split('\n');
    var existing_node_path = lines[2].split('"')[1];
    var existing_agentx_path = lines[3].split('"')[1];
    var existing_cfg_path = lines[4].split('"')[1];

    if (existing_node_path !== paths.node ||
        existing_agentx_path !== paths.agentx ||
        existing_cfg_path !== paths.cfg) {

      fs.readFile(paths.start_agentx, function (err, stdout, stderr) {
        if (err) {
          throw err;
        }
        var ll = stdout.toString().split('\n');
        ll[2] = 'DEFAULT_NODE="' + paths.node + '"';
        ll[3] = 'DEFAULT_AGENTX="' + paths.agentx + '"';
        ll[4] = 'CFG="' + paths.cfg + '"';
        fs.writeFile(paths.start_agentx + '-tmp', ll.join('\n'), function (err) {
          if (err) {
            throw err;
          }
          console.log('Run the following command will enable agentx startup automatically after system reboot.');
          console.log('    sudo ' + paths.script);
        });
      });
    }
  });
