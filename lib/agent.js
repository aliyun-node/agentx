...
Agent.prototype.run = function () {
  ...
  this.connectTimer = setTimeout(function() {
    if (that.conn.ws && that.conn.ws._socket && that.conn.ws._socket.destroy) {
      if (typeof that.conn.ws._socket.destroy === 'function') {
        that.conn.ws._socket.destroy();
      }
    }
    that.run();
  }, 10 * 1000);
...
Agent.prototype.reconnect = function () {
  var that = this;
  // delay 3 - 10s
  var delay = utils.random(3000, this.reconnectDelay);
