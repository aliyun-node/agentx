...
var Agent = function (config) {
  ...
  config.cmddir = config.cmddir || path.join(__dirname, '../commands');
...
Agent.prototype.execCommand = function (params, id) {
  ...
  var file = path.join(this.prefix, `${cmd}.js`);
  fs.access(file, function (err) {
    ...
    args.unshift(file);
    utils.execCommand('node', args, opts, function (err, stdout, stderr) {
